\section{Tema d'esame del 9 Febbraio 2026}

\subsection*{Esercizio 1}

Tradurre in logica del prim'ordine con uguaglianza le seguenti affermazioni in linguaggio naturale, utilizzando solo i simboli di relazione, di funzione e di costante aggiuntivi indicati di seguito. 
Simboli di relazione: \(G(\cdot) = \) \q{è un grafo}; \(V(\cdot, \cdot) = \) \q{è un vertice di}; \(E(\cdot, \cdot, \cdot) = \) \q{è adiacente a in}. 
Simboli di funzione: \(c(\cdot) = \) \q{il colore di}. 
Simboli di costante: \(B = \) \q{bianco}; \(N = \) \q{nero}.
\paragraph{Svolgimento:}
\begin{itemize}
  \item \textbf{(a)} I vertici di un grafo sono di colore nero solo se i loro adiacenti nel grafo sono di colore bianco.
  \[\forall g(G(g) \rightarrow \forall v((V(v,g) \land c(v)=N) \rightarrow \forall u((V(u,g) \land E(u,v,g)) \rightarrow c(u)=B)))\]

  \item \textbf{(b)} I vertici neri in un grafo non hanno come adiacenti vertici dello stesso colore.
  \[\forall g(G(g) \rightarrow \forall v((V(v,g) \land c(v)=N) \rightarrow \forall u((V(u,g) \land E(u,v,g)) \rightarrow \neg(c(v)=c(u)))))\]

  \item \textbf{(c)} In ogni grafo, alcuni vertici hanno come adiacenti solo vertici di colore bianco.
  \[\forall g(G(g) \rightarrow \exists v\st (V(v,g) \land \forall u((V(u,g) \land E(u,v,g)) \rightarrow c(u)=B)))\]

  \item \textbf{(d)} Vi sono grafi nei quali i vertici di colore nero hanno, tra i loro adiacenti, almeno due vertici di colore bianco che non sono adiacenti tra di loro.
  \[
  \begin{aligned}    
    \exists g&\st (G(g) \land \forall v((V(v,g) \land c(v)=N) \rightarrow \exists u_1\st \exists u_2\st\\
    &(V(u_1,g) \land V(u_2,g) \land E(u_1,v,g) \land E(u_2,v,g) \land c(u_1)=B \land c(u_2)=B \land \\
    &\neg E(u_1,u_2,g) \land \neg(u_1=u_2))))
  \end{aligned}
  \]

  \item \textbf{(e)} In un arbitrario grafo, almeno un vertice di colore bianco ha meno di 3 vertici neri tra i suoi adiacenti.
  \[
  \begin{aligned}
    \forall g&\st (G(g) \rightarrow \exists v\st\\
    & (V(v,g) \land c(v)=B \land \forall u_1 \forall u_2 \forall u_3 \\
    &((V(u_1,g) \land V(u_2,g) \land V(u_3,g) \land E(v,u_1,g) \land E(v,u_2,g) \land E(v,u_3,g) \land c(u_1)=N \land c(u_2)=N \\
    & \land c(u_3)=N) \rightarrow (u_1=u_2 \lor u_2=u_3 \lor u_1=u_3))))
  \end{aligned}
  \]
\end{itemize}
...
Indicare, per ciascuna delle seguenti formule, se sono valide, insoddisfacibili o solo soddisfacibili. In ciascun caso, motivare semanticamente la risposta data. Esibire, successivamente, una deduzione in Natural Deduction per ogni formula che risulta valida o per la sua negazione, se la formula risulta insoddisfacibile.
\begin{enumerate}
...
\begin{enumerate}
    ...
    \begin{prooftree}
      ...
      \RuleLabel{\(\exists E_{4}\)}
      \BinaryInfC{\(\bot\)}
      
      \AxiomC{\({[\exists y\st \exists z\st (\dots)]}_2\)}
      \RuleLabel{\(\exists E_{3}\)}
      \BinaryInfC{\(\bot\)}
