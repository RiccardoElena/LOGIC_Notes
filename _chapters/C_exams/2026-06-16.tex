\section{Tema d'esame del 16 Giugno 2026}
\subsection*{Esercizio 1}
Si traducano in logica del prim'ordine con uguaglianza le seguenti affermazioni in linguaggio naturale, utilizzando solo i simboli costanti presenti nelle frasi e i simboli di costante e di predicato indicati di seguito. Simboli di costante: 1 e 2. Simboli di predicato: \(N(\cdot)=\) \q{\(\cdot\) è un numero}; \(\Pr(\cdot)=\) \q{\(\cdot\) è un numero primo}; \(Succ(\cdot,\circ)=\) \q{\(\cdot\) è successore di \(\circ\)}; \(Div(\cdot,\circ)=\) \q{\(\cdot\) è divisibile per \(\circ\)}; \((\cdot < \circ)=\) \q{\(\cdot\) minore di \(\circ\)}.
\paragraph{Svolgimento:}
Definiamo innanzitutto l'abbreviazione \(NP(x) := N(x) \land \Pr(x)\).

\begin{itemize}
    \item \textbf{(a)} I numeri primi non formano un insieme denso.
    ...
    \item \textbf{(b)} I numeri primi maggiori di 2 hanno successore pari.
    \[ \forall x ((NP(x) \land (2 < x)) \to \exists s \st (N(s) \land Succ(s, x) \land Div(s, 2))) \]

    \item \textbf{(c)} Un numero primo è divisibile solo da 1 e da sé stesso.
    \[ \forall x (NP(x) \to \forall d (N(d) \land Div(x, d) \to ((d = 1) \lor (d = x)))) \]

    \item \textbf{(d)} I numeri primi formano un insieme discreto.
...
\subsubsection*{(b) \((a=b) \to ((c=b) \to ((\forall x \st C(a,x)) \to (\exists x \st C(c,x))))\)}
