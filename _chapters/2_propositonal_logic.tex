...
Questo poiché, preso \(\Gamma = \emptyset\), ogni modello soddisfa \(\Gamma\), poichè un modello è un insieme e l'insieme vuoto è contenuto in tutti gli insiemi.
...
Per dimostrare questo teorema, è necessario dare carattere semantico all'operazione di sostituzione, altrimenti esclusivamente sintattica, e mostrare che questa operazione preserva la proprietà di tautologia.
...
\begin{proof}
  ...
  \begin{description}
    \item[\(\phi \in \AP\):] in questo caso, che rappresenta il caso base, \(\phi\sigma = \sigma(\phi)\). Ma quindi
      \[\Val{\alpha}[\phi\sigma] = \Val{\alpha}[\sigma(\phi)] \overset{def}{=} \alpha^\sigma(\phi) = \Val{\alpha^\sigma}[\phi]\]
...
Ciò avviene perché la dimensione della tabella di verità di una formula \(\phi\) è \(2^{\AP[\phi]}\), dovendo considerare tutti gli assegnamenti possibili, e dunque la formula in \(\DNF\) o \(\CNF\) costruita a partire da tale tabella può avere un numero di clausole esponenziale. 
