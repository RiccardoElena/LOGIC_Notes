...
Vedere che le due definizioni sono equivalenti è immediato, poiché se \(\Gamma \vdash \bot\), allora per \(\bot E\) si ha che \(\Gamma \vdash \phi\) per ogni formula \(\phi\). Viceversa, se \(\Gamma \vdash \phi\) per ogni formula \(\phi\), allora in particolare si ha che \(\Gamma \vdash \bot\).
